...
def _train_and_evaluate(
    *,
    ...
    num_sub_iterations: int | None = None,
) -> dict[str, object]:
    ...
    if num_sub_iterations is not None:
        cfg_kwargs["num_sub_iterations"] = num_sub_iterations
...
def run_experiment(
    *,
    ...
    initial_vocab_factor: int | None = None,
    max_token_len: int | None = None,
    num_sub_iterations_list: list[int] | None = None,
    use_cache: bool = True,
) -> list[dict[str, object]]:
    """Run training experiments serially with caching and return results.

    This experiment sweeps only the num_sub_iterations option.
    """
    logger = create_logger("experiment", verbose=True)
    results: list[dict[str, object]] = []

    if not num_sub_iterations_list:
        num_sub_iterations_list = [1, 2, 3, 5]

    all_jobs = [
        (corpus_name, algo, nsi)
        for corpus_name in corpus_names
        for algo in init_algorithms
        for nsi in num_sub_iterations_list
    ]

    for corpus_name, algo, nsi in all_jobs:
        cache_dir = Path("results") / "mce_final_style_prune" / corpus_name
        cache_dir.mkdir(parents=True, exist_ok=True)

        experiment_parts = [algo, f"n{additional_vocab_size}", f"nsi{nsi}"]
        if initial_vocab_factor is not None:
        ...
        if use_cache and cache_file.exists():
            logger.info(f"💾 Loading cached result for {corpus_name}:{algo} (nsi={nsi}) from {cache_file}")
        ...
        logger.info(
            f"▶️  Starting {algo} on {corpus_name} (n={additional_vocab_size}, nsi={nsi})"
        )
        ...
        try:
            result = _train_and_evaluate(
                ...
                num_sub_iterations=nsi,
            )
            result["algo"] = algo
            result["num_sub_iterations"] = nsi
            result["final_style_prune"] = True
            results.append(result)
            ...
            logger.error(f"❌ Job failed for {corpus_name}:{algo} (nsi={nsi})", exc_info=True)
...
if __name__ == "__main__":
    ...
    results = run_experiment(
        ...
        num_sub_iterations_list=[int(sys.argv[1])],
    )
