...
Why this exists instead of reusing `paper_utils/boundary/tokenizers/*`
-------------------------------------------------------------------
...
from paper_utils.boundary.boundary_pretokenizer import BOUNDARY_VARIANTS, get_boundary_pretokenizer
...
def main(
    ...
    Args:
        ...
        eval_texts: Optional JSON list of held-out texts for a chars/token sanity check.
            `paper_utils/boundary/eval_texts/en.json` is the compression grid's slice.
    ...
    for arm in [a.strip() for a in arms.split(",") if a.strip()]:
        assert arm == "plain" or arm in BOUNDARY_VARIANTS, f"unknown arm {arm!r}"
