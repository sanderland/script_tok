...
class UnigramTrainerConfig(TrainerConfig):
    ...
    init_vocab_algo: str | tuple = (
        "corpus_long"  # one of {"simple", "corpus_long", "corpus_intermediate", "corpus_fallback"}
    )
...
class UnigramTrainer(BaseTrainer):
    ...
    def train(self) -> UnigramModel:
        ...
        model.metadata = {
            **stats,
            "config": self.config.model_dump(exclude={"forced_initial_vocab"}),
        }
        return model
    ...
    def make_initial_vocab(self) -> tuple[list[UnigramToken], int]:
        ...
        if isinstance(algo, tuple):  # TODO: remove awful hack to compare init algorithms without pretokenizer
            algo, nocb_corpus = algo
            self.logger.info(f"🔍 Creating initial vocabulary using {algo} algorithm, no pretokenizer")
            self.config.init_vocab_algo = algo  # cant save corpus in config, so we need to save the algo
