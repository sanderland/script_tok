...
def _train_and_evaluate(
    *,
    ...
    m_step_low_count_threshold: float | None = None,
) -> dict[str, object]:
    ...
    if m_step_low_count_threshold is not None:
        cfg_kwargs["m_step_low_count_threshold"] = m_step_low_count_threshold
...
def run_experiment(
    *,
    ...
    initial_vocab_factor: int | None = None,
    max_token_len: int | None = None,
    low_count_thresholds: list[float] | None = None,
    use_cache: bool = True,
) -> list[dict[str, object]]:
    """Run training experiments serially with caching and return results.

    This experiment sweeps only the m_step_low_count_threshold option.
    """
    logger = create_logger("experiment", verbose=True)
    results: list[dict[str, object]] = []

    if not low_count_thresholds:
        low_count_thresholds = [0.0, 0.5, 2.0, 10.0]

    all_jobs = [
        (corpus_name, algo, thr)
        for corpus_name in corpus_names
        for algo in init_algorithms
        for thr in low_count_thresholds
    ]

    for corpus_name, algo, thr in all_jobs:
        cache_dir = Path("results") / "mce_low_count_threshold" / corpus_name
        cache_dir.mkdir(parents=True, exist_ok=True)

        experiment_parts = [algo, f"n{additional_vocab_size}", f"lct{thr}"]
        if initial_vocab_factor is not None:
        ...
        if use_cache and cache_file.exists():
            logger.info(f"💾 Loading cached result for {corpus_name}:{algo} (lct={thr}) from {cache_file}")
        ...
        logger.info(
            f"▶️  Starting {algo} on {corpus_name} (n={additional_vocab_size}, lct={thr})"
        )
        ...
        try:
            result = _train_and_evaluate(
                ...
                m_step_low_count_threshold=thr,
            )
            result["algo"] = algo
            result["m_step_low_count_threshold"] = thr
            results.append(result)
            ...
            logger.error(f"❌ Job failed for {corpus_name}:{algo} (lct={thr})", exc_info=True)
