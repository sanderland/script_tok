#!/usr/bin/env python3
"""Merge the per-cell tokenizer manifests into one, and check the vocabularies match.

`train_multilang.py` writes one fragment per trained cell rather than updating a shared
manifest, because that file is rewritten read-modify-write and two languages training at
once -- on two machines, or in two shells -- would drop each other's entries. This folds
the fragments into a single manifest.json and applies the cross-arm check a single-process
run of train_matched.py does at the end: every arm of the same trainer and corpus must
land on the same total vocabulary, since that is the whole point of matched training.
