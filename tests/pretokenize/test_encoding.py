...
from script_bpe.pretokenize.scriptencoding import END_CODEPOINT, ScriptEncodingV1, ScriptEncodingV2, unicode_script_map
...
@pytest.mark.parametrize("script_encoding", [ScriptEncodingV1, ScriptEncodingV2])
def test_build_script_encoding(script_encoding):
    blocks = script_encoding.blocks
    assert isinstance(blocks, list)

    # Compute simple metadata expected by the test from the ScriptConfig instance
    num_index_tokens = max((len(b.chars) for b in blocks), default=0)
    num_blocks = len(blocks)
    assert isinstance(num_index_tokens, int)
