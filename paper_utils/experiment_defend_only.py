...
def _train_and_evaluate(
    *,
    ...
    defensive_prune: bool | None = None,
) -> dict[str, object]:
    ...
    if defensive_prune is not None:
        cfg_kwargs["defensive_prune"] = defensive_prune
...
def run_experiment(
    *,
    ...
    initial_vocab_factor: int | None = None,
    max_token_len: int | None = None,
    defensive_prune_options: list[bool] | None = None,
    use_cache: bool = True,
) -> list[dict[str, object]]:
    """Run training experiments serially with caching and return results.

    This experiment sweeps only the defensive pruning option.
    """
    logger = create_logger("experiment", verbose=True)
    results: list[dict[str, object]] = []

    if not defensive_prune_options:
        defensive_prune_options = [False, True]

    all_jobs = [
        (corpus_name, algo, defend)
        for corpus_name in corpus_names
        for algo in init_algorithms
        for defend in defensive_prune_options
    ]

    for corpus_name, algo, defend in all_jobs:
        cache_dir = Path("results") / "mce_defend" / corpus_name
        cache_dir.mkdir(parents=True, exist_ok=True)

        experiment_parts = [algo, f"n{additional_vocab_size}", f"def{int(defend)}"]
        if initial_vocab_factor is not None:
        ...
        if use_cache and cache_file.exists():
            logger.info(f"💾 Loading cached result for {corpus_name}:{algo} (defend={defend}) from {cache_file}")
        ...
        logger.info(
            f"▶️  Starting {algo} on {corpus_name} (n={additional_vocab_size}, defend={defend})"
        )
        ...
        try:
            result = _train_and_evaluate(
                ...
                defensive_prune=defend,
            )
            result["algo"] = algo
            result["defensive_prune"] = defend
            results.append(result)
            ...
            logger.error(f"❌ Job failed for {corpus_name}:{algo} (defend={defend})", exc_info=True)
...
if __name__ == "__main__":
    # Base algorithms only (no alias modifiers); we will sweep defensive_prune
