...
class BaseTokenizer:
    ...
    def report(self, n_longest: int = 20) -> str:
        ...
        longest_atomic_rows = [
            {
                "ID": t.id,
                "Atomic Tokens": len(t.atomic_tokens),
                "Text": repr(self.pretokenizer.tokens_repr(t.atomic_tokens)),
            }
        ...
        longest_chars_rows = [
            {
                "ID": t.id,
                "Characters": len(self.decode([t.id])),
                "Text": repr(self.pretokenizer.tokens_repr(t.atomic_tokens)),
            }
    ...
    def corpus_performance(
        ...
        for atomic_tokens, count in corpus:
            ...
            total_byte_len += len(text.encode("utf-8")) * count
        ...
        return {
            ...
            "bytes_per_token": safe_div(total_byte_len, total_tokens_len),
            "bytes_per_char": safe_div(total_byte_len, total_char_len),
            "renyi_bits": renyi,
