#!/usr/bin/env python3
"""Why the downstream sweep scores bits-per-byte and not CORE.
...
nothing at all, bits-per-byte included. Scoring only the arms that survive would compare
a subset of the sweep against itself, so `run_arms.sh` scores no arm on CORE and the
paper reports bits-per-byte. This script is the measurement behind that choice.
...
seed), so the prompts are the ones base_eval would actually tokenize.
    ...
    uv run python paper_utils/boundary/downstream/core_prefix_check.py \
        --tokenizer-dir paper_utils/boundary/downstream/tokenizers
...
def main(
    ...
    from paper_utils.boundary.downstream.boundary_tokenizer import (
    ...
    failures = 0
    print(f"\n  {'tokenizer':<52} {'aborts':>12}  verdict")
    for name in names:
        try:
        ...
        for pairs in per_task.values():
            for without, with_ in pairs:
                ...
                if not (len(a) < len(b) and a == b[: len(a)]):
                    bad += 1
        verdict = "ok" if bad == 0 else "breaks CORE"
        failures += bad > 0
        print(f"  {name:<52} {bad:>5}/{total:<6}  {verdict}")

    print(
        f"\n{failures} of {len(names)} tokenizer(s) cannot be scored on CORE, "
        f"which is why the sweep scores bits-per-byte instead."
    )
