...
class ScriptConfig(BaseModel):
    ...
    def __post_model_init__(self):
        ...
        assert self.config.largest_block in chars_by_sc, f"{self.config.largest_block} not found. Blocks are: {chars_by_sc.keys()}"
        num_index_tokens = len(chars_by_sc[self.config.largest_block])
        self.blocks = []
        for sc, cps in sorted(chars_by_sc.items(), key=lambda kv: (num_chars_by_script[kv[0][0]], kv[1]), reverse=True):
            ...
            for sub_block, start in enumerate(range(0, len(cps), num_index_tokens)):
                self.blocks.append( ScriptBlock(script_id=sid, script=script, category=supercat, sub_block_id=sub_block, combines_with_spaces=combines_with_spaces, chars="".join(cps[start : start + num_index_tokens]) ) )

        if self.merge_hiragana_with_han:
            self.blocks = self._merge_hiragana_with_han(self.blocks)
        return self.blocks, {}
...
ScriptEncodingV1 = ScriptConfig(supercategory_type="V1", script_cat_override=V1_SCRIPT_CAT_OVERRIDE | DEFAULT_SCRIPT_CAT_OVERRIDE)
ScriptEncodingV2 = ScriptConfig(supercategory_type="V2", script_cat_with_spaces=DEFAULT_SCRIPT_WITH_SPACES + [(ScriptConfig.ASCII_SCRIPT, "PSF")])
