...
from script_bpe.tokenizers.mingram.trainer_careful import CarefulMinGramTrainer, CarefulMinGramTrainerConfig
...
def test_careful_mingram_trainer_smoke(tmp_path):
    pretokenizer = get_pretokenizer("bytes_gpt4_cb")
    text = ("abracadabra abracadabra\n" * 6) + ("banana bandana banana\n" * 6)
    corpus = PretokenizedCorpus.from_texts(
        "test_careful_mingram_smoke",
        texts=[text],
        pretokenizer=pretokenizer,
        base_path=str(tmp_path),
    )
    config = CarefulMinGramTrainerConfig(
        additional_vocab_size=10,
        overshoot_factor=2.0,
        pruning_shrinking_factor=0.5,
        verbose=False,
    )
    model = CarefulMinGramTrainer(pretokenizer, corpus, config).train()
