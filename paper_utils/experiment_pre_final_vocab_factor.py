...
def _train_and_evaluate(
    *,
    ...
    pre_final_vocab_factor: float | None = None,
) -> dict[str, object]:
    ...
    if pre_final_vocab_factor is not None:
        cfg_kwargs["pre_final_vocab_factor"] = pre_final_vocab_factor
    ...
    return {
        ...
        "pre_final_vocab_factor": cfg.pre_final_vocab_factor,
    }
...
def run_experiment(
    *,
    ...
    initial_vocab_factor: int | None = None,
    max_token_len: int | None = None,
    pre_final_vocab_factors: list[float] | None = None,
    use_cache: bool = True,
    ...
    if not pre_final_vocab_factors:
        pre_final_vocab_factors = [1.1]

    all_jobs = [
        (corpus_name, algo, pfvf)
        for corpus_name in corpus_names
        for algo in init_algorithms
        for pfvf in pre_final_vocab_factors
    ]

    for corpus_name, algo, pfvf in all_jobs:
        cache_dir = Path("results") / "mce" / corpus_name
        cache_dir.mkdir(parents=True, exist_ok=True)

        experiment_parts = [algo, f"n{additional_vocab_size}", f"pfvf{pfvf}"]
        if initial_vocab_factor is not None:
        ...
        logger.info(
            f"▶️  Starting {algo} on {corpus_name} (n={additional_vocab_size}, pfvf={pfvf})"
        )
        ...
        try:
            result = _train_and_evaluate(
                ...
                pre_final_vocab_factor=pfvf,
            )
            result["algo"] = algo
            result["pre_final_vocab_factor"] = pfvf
            results.append(result)
...
if __name__ == "__main__":
    ...
    results = run_experiment(
        ...
        pre_final_vocab_factors=[1.0, 1.01, 1.1, 1.25, 1.5, 2.0, 5.0, 10.0],
    )
