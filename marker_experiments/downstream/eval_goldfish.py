...
----------------------------------------------
`train_multilang.py` defaults `--eval-texts` to `marker_experiments/eval_texts/{lang}.json`,
the *FineWiki* held-out documents the older compression grids withheld. Those tokenizers
train on `fineweb_{lang}_5gb`, so that pairing is cross-domain by accident rather than by
design, and it is also not one measurement: the slices actually used differ per language
(ar/de/ru match `finewiki1gb_grid.py`, en and fi match no script still in the repo), the
directory is gitignored, and a third of the grid -- every `quick` cell, and all of Korean
-- was trained on machines that had no slice at all and so recorded no number.
...
axis the boundary marker changes.
    ...
    uv run python marker_experiments/downstream/eval_goldfish.py
