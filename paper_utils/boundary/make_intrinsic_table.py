...
vocabulary, evaluated on held-out Goldfish. The grid is built on the `quick` sample, which
reads until its character budget is full rather than reservoir-sampling the whole source --
non-uniform, but hours rather than days per language, and it is what the paper reports.
...
Needs `booktabs` and the paper's boundary macros. The `\textuparrow` key is the case form,
which the scheme places outside the span's markers:
    ...
    uv run python paper_utils/boundary/make_intrinsic_table.py
    uv run python paper_utils/boundary/make_intrinsic_table.py --layout mean
"""
...
from paper_utils.boundary.utils import GENERATED, rel
...
# case-coded form. The case codes sit outside the span's markers and are applied only to a
# span that carries case; the two earlier schemes -- codes inside the markers, and codes
# outside but with no case test -- are not in the pretokenizer any more and not reported.
ARM_ORDER = ["bnd_w", "bnd_w_caps", "bnd_wp", "bnd_wp_caps", "bnd_wpd", "bnd_wpd_caps"]
ARM_LABEL = {
    "bnd_w": r"\bnds{w}",
    "bnd_wp": r"\bnds{wp}",
    "bnd_wpd": r"\bnds{wpd}",
    "bnd_w_caps": r"\bnds{wcaps}",
    "bnd_wp_caps": r"\bnds{wpcaps}",
...
MAIN_ARMS = ["bnd_w", "bnd_wp", "bnd_wpd", "bnd_wpd_caps"]
...
def main(
    ...
    with open(results) as f:
        data = json.load(f)
    cells = read_cells(data)
    langs, trainers, available = select(cells)
    if not langs:
        raise SystemExit(f"no usable cells in {results}")
    ...
    suffix = "_main" if layout == "mean" else "_quick"
    ...
    detail = detail or "tab:intrinsic-quick"
    setting = setting or (
        "trained on 5\\,GB of FineWeb per language"
        + " (\\texttt{quick} non-uniform sample)"
    ...
    tex = "\n".join([
        "% Generated by paper_utils/boundary/make_intrinsic_table.py. Do not edit.",
        r"% Requires booktabs, xcolor and the paper's \bnds and \plainscheme macros.",
        f"% source: {rel(results)} (quick corpus)",
        r"\centering",
