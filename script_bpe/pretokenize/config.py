...
class ScriptCharEnc:
    __slots__ = ('script_id', 'combines_with_spaces', 'atomic_tokens', 'inherited')
    def __init__(self, block: ScriptBlock, token_pair: TokenPairT):
        ...
        self.atomic_tokens = token_pair
        self.inherited = block.script == "Inherited"

    def __repr__(self):
        return f"ScriptCharEnc(script_id={self.script_id}, atomic_tokens={self.atomic_tokens})"


class UTF8CharEnc:
    __slots__ = ('atomic_tokens')
    def __init__(self, byte_token: int):
        self.atomic_tokens = [byte_token]

    def __repr__(self):
        return f"UTF8CharEnc(byte_token={self.atomic_tokens[0]})"
...
class Pretokenizer:
    ...
    def pretokenize(self, text: str) -> PretokenizedT:
        ...
        text_chunks = self.split_unencoded_and_encode(text)
        text_chunks = self.split_encoded(text_chunks)
        return [[tid for tid in ch.atomic_token_ids] for ch in text_chunks]
    ...
    def split_unencoded_and_encode(self, text: str) -> list[str]:
        ...
        for i in range(len(encoded_chunks)):
            if i % 2 == 0 or self.config.digit_handling is None: # non-digits
                ...
                encoded_chunks[i] = [self.encode_digits(c) for c in group_digits(encoded_chunks[i], self.config.digit_handling)]
...
class UTF8Pretokenizer(Pretokenizer):
    ...
    def decode(self, base_token_ids: InputTokenSeq, errors="replace") -> str:
        bytes = bytes([self.token_to_byte[id] for id in base_token_ids])
        return bytes.decode("utf-8", errors=errors)
...
class ScriptPretokenizer(Pretokenizer):
    ...
    def split_encoded(self, encoding: list[ScriptCharEnc]) -> list[list[ScriptCharEnc]]:
        ...
        """
        if not self.config.split_encoded:
            return [encoding]
...
if __name__ == "__main__":
    ...
    print(pt.pretokenize("hello world"))
